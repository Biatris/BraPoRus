...
speakers = ('AVM:', 'EAB:', 'IMK:', 'NKP:', 'SAP:', 'SSN:', 'TSL:', 'VPK:', 'ZPP:', 'EKS:', 'GAA:', 'MVB:')
...
sentence_tokens = [[token.text for token in sent if not token.is_punct and not token.is_space] for sent in doc.sents]

with open("sentence_tokens_test.csv", "w") as f:
    wr = csv.writer(f)
